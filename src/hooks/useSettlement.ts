...
declare global {
  interface Window {
    electronAPI?: {
      getSettlement: (id: string) => Promise<SettlementData>;
      updateSettlement: (id: string, data: SettlementData) => Promise<boolean>;
      updateField: (id: string, field: string, value: any) => Promise<boolean>;
      getResourceChains: (id: string) => Promise<any>;
      getMilitaryStatus: (id: string) => Promise<any>;
      getHistoricalData: (id: string) => Promise<any>;
      chatWithClaude: (message: string, settlementId: string) => Promise<string>;
    };
  }
}

// Fallback data for browser-only dev mode
const fallbackData: SettlementData = {
  id: 'default',
...
export function useSettlement(id: string = 'default') {
  const [data, setData] = useState<SettlementData>(fallbackData);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (window.electronAPI) {
        const result = await window.electronAPI.getSettlement(id);
  ...
  const updateField = useCallback(
    async (field: string, value: any) => {
      if (window.electronAPI) {
        await window.electronAPI.updateField(id, field, value);
        await refresh();
      } else {
  ...
  return { data, loading, refresh, updateField };
}
